...
import { getScheduleByDay, getAllCategories, getAllTags } from '@/data/scheduleData';
...
const Schedule = () => {
  ...
  const currentDaySchedule = scheduleByDay.find(day => day.name === selectedDay);
  const showsForSelectedDay = currentDaySchedule ? currentDaySchedule.shows : [];
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-12">
      ...
      <div className="mb-8">
        <Tabs defaultValue="grid" className="w-full" onValueChange={(val) => setViewMode(val as "grid" | "timeline")}>
      ...
      <div className="mt-16">
        <h2 className="font-display text-2xl font-bold mb-6">Weekly Overview</h2>
                ...
                <div key={day.name} className="text-center">
                  <div 
                    className={`font-medium p-2 mb-2 rounded-lg ${
                      day.name === selectedDay ? 'bg-[#2295e2] text-white' : 'bg-gray-100 text-black'
                    }`}
                  >
                    {day.name}
                  </div>
                  <div className="space-y-2">
                    {day.shows.map((show) => (
                      ...
                      >
                        ...
                        <p className="text-gray-500">{show.startTime.substring(0, 5)}</p>
                      </div>
                    ))}
                    {day.shows.length === 0 && (
